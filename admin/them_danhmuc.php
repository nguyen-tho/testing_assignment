...
if(isset($_POST['btnthem']))
{
	$tendm=$_POST['tendm'];
	$dequi=$_POST['dequi'];
	$insertdm = mysqli_query($conn,"INSERT INTO danhmuc VALUES('', '$tendm', '$dequi') ");
	if($insertdm) {
			...
			<td>
            	<select name="dequi">
                    ...
                    <?php
						...
						while($show1 = mysqli_fetch_assoc($show))
						{
							$madm = $show1['madm'];	
							$tendm = $show1['tendm'];
							echo "<option value='".$madm."'>".$tendm."</option>";	
								$show2 = mysqli_query($conn,"SELECT * FROM danhmuc WHERE dequi='".$madm."'");
								while($show3 = mysqli_fetch_assoc($show2))
								{
									$madm1 = $show3['madm'];	
									$tendm1 = $show3['tendm'];
